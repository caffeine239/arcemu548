...
class DalliahTheDoomsayerAI : public MoonScriptBossAI
{
	public:
		...
		void OnDied(Unit* mKiller)
		{
			...
			if(door2 != NULL)
				door2->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 0);
...
class WrathScryerSoccothratesAI : public MoonScriptBossAI
{
	public:
		...
		void OnDied(Unit* mKiller)
		{
			...
			if(door1 != NULL)
				door1->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 0);
...
class WardenMellicharAI : public MoonScriptBossAI
{
	public:
		...
		void OnCombatStart(Unit* mTarget)
		{
			...
			if(shield)
				shield->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 1);
		...
		void AIUpdate()
		{
			...
			if(IsTimerFinished(Phase_Timer) && GetPhase() == 0)
			{
				if(Phasepart == 0)
				{
					...
					if(orb1)
						orb1->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 0);
			...
			else if(IsTimerFinished(Phase_Timer) && GetPhase() == 1)
			{
				if(Phasepart == 0)
				{
					...
					if(orb2)
						orb2->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 0);
			...
			else if(IsTimerFinished(Phase_Timer) && GetPhase() == 2)
			{
				if(Phasepart == 0)
				{
					...
					if(orb3)
						orb3->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 0);
			...
			else if(IsTimerFinished(Phase_Timer) && GetPhase() == 3)
			{
				if(Phasepart == 0)
				{
					...
					if(orb4)
						orb4->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 0);
		...
		void Reset_Event()
		{
			...
			if(shield)
				shield->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 0);

			if(orb1)
				orb1->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 1);

			if(orb2)
				orb2->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 1);

			if(orb3)
				orb3->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 1);

			if(orb4)
				orb4->SetByte(GAMEOBJECT_FIELD_STATE_SPELL_VISUAL_ID, 0, 1);
