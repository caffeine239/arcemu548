...
{
    sLog.outError("Incorrect sequence element %d detected at %s", element, function);
    return false;
}
...
void Movement::PacketSender::Send() const
{
    ...
    if (Player* player = _unit->ToPlayer())
    {
		...
		if (isPlayerMovement && _selfOpcode != MSG_NULL_ACTION)
        {
