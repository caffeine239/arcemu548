...
void Vehicle::Load( Unit *owner, uint32 creature_entry, uint32 vehicleid ){
	...
	vehicle_info = dbcVehicleEntry.LookupEntry( vehicleid );
	...
	for( uint32 i = 0; i < MAX_VEHICLE_SEATS; i++ ){
		...
		if( seatid != 0 ){
			VehicleSeatEntry *seatinfo = dbcVehicleSeatEntry.LookupEntry( seatid );
			if( seatinfo == NULL ){
...
void Vehicle::AddPassengerToSeat( Unit *passenger, uint32 seatid ){
	...
	if( !HasEmptySeat() ){
		...
		owner->RemoveFlag( UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_PLAYER_VEHICLE );
...
void Vehicle::EjectPassengerFromSeat( uint32 seatid ){
	...
	if( HasEmptySeat() ){
		if( owner->IsPlayer() )
			owner->SetFlag( UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_PLAYER_VEHICLE );
		else
			owner->SetFlag( UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK );
	}
