...
bool ChatHandler::HandleGOInfo(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(m_session, "%s dynflags:%s%u", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_FIELD_PERCENT_HEALTH));
...
bool ChatHandler::HandleGOEnable(const char* args, WorldSession* m_session)
{
	...
	if(GObj->GetUInt32Value(GAMEOBJECT_FIELD_PERCENT_HEALTH) == 1)
	{
		// Deactivate
		GObj->SetUInt32Value(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0);
		BlueSystemMessage(m_session, "Gameobject deactivated.");
	}
	else
	{
		// /Activate
		GObj->SetUInt32Value(GAMEOBJECT_FIELD_PERCENT_HEALTH, 1);
