...
bool ChatHandler::HandleStartCommand(const char* args, WorldSession* m_session)
{
	...
	if(strlen(args) > 0)
	{
		...
		if(race == "human")
			...
			raceid = 11;
		else if(race == "deathknight" || race == "dk") // this is not a hackfix, used for teleporting to ebon hold
			classid = 6;
		else if(race == "worgen")
			raceid = 22;
		else
		{
			RedSystemMessage(m_session, "Invalid start location! Valid locations are: human, dwarf, gnome, nightelf, draenei, orc, troll, tauren, undead, bloodelf, deathknight, worgen, goblin.");
			return true;
		}
	}
	else
...
bool ChatHandler::HandleGMListCommand(const char* args, WorldSession* m_session)
{
	...
	for(itr = objmgr._players.begin(); itr != objmgr._players.end(); ++itr)
	{
		if(itr->second->GetSession()->GetPermissionCount())
		{
			if(isGM || !sWorld.gamemaster_listOnlyActiveGMs || (sWorld.gamemaster_listOnlyActiveGMs && itr->second->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAG_GM)))
			{
				if(first)
				...
				{
					if(sWorld.gamemaster_listOnlyActiveGMs && !itr->second->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAG_GM))
