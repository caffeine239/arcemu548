...
Item* ItemInterface::SafeAddItem(uint32 ItemId, int8 ContainerSlot, int16 slot)
{
	...
	if(pProto->InventoryType == INVTYPE_BAG)
	{
		...
		TO< Container* >(pItem)->Create(ItemId, m_pOwner);
...
AddItemResult ItemInterface::m_AddItem(Item* item, int8 ContainerSlot, int16 slot)
{
	...
	for(i = 0; i < MAX_INVENTORY_SLOT; ++i)
	{
		...
		if(tempitem != NULL)
		{
			...
			if(tempitem->IsContainer())
			{
				...
				for(j = 0; j < k; ++j)
				{
					if(TO< Container* >(tempitem)->GetItem(static_cast<int16>(j)) == item)
					{
#ifdef WIN32
	...
	else //case 2: item is from a bag container
	{
		...
		{
			bool result = TO< Container* >(m_pItems[(int)ContainerSlot])->AddItem(slot, item);
			if(!result)
			{
				return ADD_ITEM_RESULT_ERROR;
			}
...
Item* ItemInterface::SafeRemoveAndRetreiveItemFromSlot(int8 ContainerSlot, int16 slot, bool destroy)
{
	...
	else
	{
		...
		{
			pItem = TO< Container* >(pContainer)->SafeRemoveAndRetreiveItemFromSlot(slot, destroy);
		}
	}
...
bool ItemInterface::SafeFullRemoveItemFromSlot(int8 ContainerSlot, int16 slot)
{
	...
	else
	{
		...
		if(pContainer && pContainer->IsContainer())
		{
			TO< Container* >(pContainer)->SafeFullRemoveItemFromSlot(slot);
		}
	}
	return true;
}
...
Item* ItemInterface::GetInventoryItem(int8 ContainerSlot, int16 slot)
{
	...
	else
	{
		if(IsBagSlot(ContainerSlot))
		{
			...
			{
				return TO< Container* >(m_pItems[(int)ContainerSlot ])->GetItem(slot);
			}
		}
	}
	return NULL;
...
int16 ItemInterface::GetBagSlotByGuid(uint64 guid)
{
	...
	for(uint16 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; ++i)
	{
		if(m_pItems[i] && m_pItems[i]->IsContainer())
		{
			...
			{
				Item* inneritem = (TO< Container* >(m_pItems[i]))->GetItem(static_cast<int16>(j));
...
AddItemResult ItemInterface::AddItemToFreeSlot(Item* item)
{
			...
			for(i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; i++)
			{
				if(m_pItems[i])
				{
					if(m_pItems[i]->GetProto()->BagFamily & item->GetProto()->BagFamily)
					{
						...
						{
							uint32 r_slot;
							result2 = TO< Container* >(m_pItems[i])->AddItemToFreeSlot(item, &r_slot);
							if(result2)
							{
	...
	for(i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; i++)
	{
		if(m_pItems[i] != NULL && m_pItems[i]->GetProto()->BagFamily == 0 && m_pItems[i]->IsContainer()) //special bags ignored
		{
			for(uint32 j = 0; j < m_pItems[i]->GetProto()->ContainerSlots; j++)
			{
				Item* item2 = (TO< Container* >(m_pItems[i]))->GetItem(static_cast<int16>(j));
				if(item2)
		...
		case BANK_SLOT_BAG_7:
			{
				...
				if(!GetInventoryItem(INVENTORY_SLOT_NOT_SET, slot))
				{
					...
					bytes = m_pOwner->GetUInt32Value(PLAYER_FIELD_REST_STATE);
...
int8 ItemInterface::CanAffordItem(ItemPrototype* item, uint32 amount, Creature* pVendor)
{
	...
	if(ex != NULL)
	{
		...
		if(m_pOwner->GetArenaCurrency() < (ex->arena * amount))
			...
			//only honor and arena currency are removed?
...
void ItemInterface::EmptyBuyBack()
{
	for(uint32 j = 0; j < MAX_BUYBACK_SLOT; j++)
	{
		if(m_pBuyBack[j] != NULL)
		{
			...
			{
				if(TO< Container* >(m_pBuyBack[j])->IsInWorld())
					TO< Container* >(m_pBuyBack[j])->RemoveFromWorld();

				delete TO< Container* >(m_pBuyBack[j]);
			}
			else
			{
			...
			m_pOwner->SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + (2 * j), 0);
			m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE + j, 0);
			m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_TIMESTAMP + j, 0);
			m_pBuyBack[j] = NULL;
		}
		else
			break;
...
{
	int i;
	if((m_pBuyBack[MAX_BUYBACK_SLOT - 1] != NULL) && (m_pOwner->GetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + (MAX_BUYBACK_SLOT - 1) * 2) != 0))
	{
		if(m_pBuyBack[0] != NULL)
		{
			...
			{
				if(TO< Container* >(m_pBuyBack[0])->IsInWorld())
					TO< Container* >(m_pBuyBack[0])->RemoveFromWorld();

				delete TO< Container* >(m_pBuyBack[0]);
			}
			else
			{
		...
		{
			//SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + (2*j),buyback[j+1]->GetGUID());
			m_pOwner->SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + (2 * j), m_pOwner->GetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + ((j + 1) * 2)));
			m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE + j, m_pOwner->GetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE + j + 1));
			m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_TIMESTAMP + j, m_pOwner->GetUInt32Value(PLAYER_FIELD_BUYBACK_TIMESTAMP + j + 1));
			m_pBuyBack[j] = m_pBuyBack[j + 1];
		}
		m_pBuyBack[MAX_BUYBACK_SLOT - 1] = it;

		m_pOwner->SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + (2 * (MAX_BUYBACK_SLOT - 1)), m_pBuyBack[MAX_BUYBACK_SLOT - 1]->GetGUID());
		m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE + MAX_BUYBACK_SLOT - 1, price);
		m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_TIMESTAMP + MAX_BUYBACK_SLOT - 1, (uint32)UNIXTIME);
		return;
	}

	for(i = 0; i <= (MAX_BUYBACK_SLOT - 1) * 2; i += 2) //at least 1 slot is empty
	{
		if((m_pOwner->GetUInt32Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + i) == 0) || (m_pBuyBack[i / 2] == NULL))
		{
			LOG_DETAIL("setting buybackslot %u", i / 2);
			m_pBuyBack[i >> 1] = it;

			m_pOwner->SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + i, m_pBuyBack[i >> 1]->GetGUID());
			//SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + i,it->GetGUID());
			m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE + (i >> 1), price);
			m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_TIMESTAMP + (i >> 1), (uint32)UNIXTIME);
			return;
		}
	}
}
...
void ItemInterface::RemoveBuyBackItem(uint32 index)
{
	...
	{
		if(m_pOwner->GetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + (j * 2)) != 0)
		{
			m_pOwner->SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + (2 * j), m_pOwner->GetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + ((j + 1) * 2)));
			m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE + j, m_pOwner->GetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE + j + 1));
			m_pOwner->SetUInt32Value(PLAYER_FIELD_BUYBACK_TIMESTAMP + j, m_pOwner->GetUInt32Value(PLAYER_FIELD_BUYBACK_TIMESTAMP + j + 1));

			if(m_pBuyBack[j + 1] != NULL && (m_pOwner->GetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOTS + ((j + 1) * 2)) != 0))
			{
				m_pBuyBack[j] = m_pBuyBack[j + 1];
			}
			else
...
void ItemInterface::SwapItemSlots(int8 srcslot, int8 dstslot)
{
	...
	if(DstItem == NULL && SrcItem != NULL && SrcItem->IsContainer())
	{
		...
		{
			tSrcItem = (TO< Container* >((m_pItems[(int)srcslot])))->GetItem(static_cast<int16>(Slot));
	...
	if(DstItem != NULL && SrcItem != NULL && SrcItem->IsContainer() && DstItem->IsContainer())
	{
		...
		{
			tSrcItem = (TO< Container* >((m_pItems[(int)srcslot])))->GetItem(static_cast<int16>(Slot));
			tDstItem = (TO< Container* >((m_pItems[(int)dstslot])))->GetItem(static_cast<int16>(Slot));
			...
			if(tSrcItem != NULL)
				(TO< Container* >(DstItem))->AddItem(static_cast<int16>(Slot), tSrcItem);
			if(tDstItem != NULL)
				(TO< Container* >(SrcItem))->AddItem(static_cast<int16>(Slot), tDstItem);
		}
	}
	...
	if(srcslot < INVENTORY_SLOT_BAG_END)	// source item is equipped
	{
		if(m_pItems[(int)srcslot])   // dstitem goes into here.
		{
			...
			{
				int VisibleBase = PLAYER_FIELD_VISIBLE_ITEMS + (srcslot * PLAYER_VISIBLE_ITEM_LENGTH);
	...
	if(dstslot < INVENTORY_SLOT_BAG_END)     // source item is inside inventory
	{
		if(m_pItems[(int)dstslot] != NULL)   // srcitem goes into here.
		{
			...
			{
				int VisibleBase = PLAYER_FIELD_VISIBLE_ITEMS + (dstslot * PLAYER_VISIBLE_ITEM_LENGTH);
		...
		do
		{
			...
			if(proto != NULL)
			{
				if(proto->InventoryType == INVTYPE_BAG)
				{
					...
					TO< Container* >(item)->LoadFromDB(fields);
...
void ItemInterface::mSaveItemsToDatabase(bool first, QueryBuffer* buf)
{
	...
	for(x = EQUIPMENT_SLOT_START; x < CURRENCYTOKEN_SLOT_END; ++x)
	{
		if(GetInventoryItem(x) != NULL)
		{
			...
			{
				TO< Container* >(GetInventoryItem(x))->SaveBagToDB(static_cast<int8>(x), first, buf);
			}
			else
			{
	...
	for(uint32 i = BANK_SLOT_BAG_START; i < BANK_SLOT_BAG_END; i++)
	{
		if(m_pItems[i] != NULL)
		{
			if(m_pItems[i]->GetProto()->BagFamily & item->GetProto()->BagFamily)
			{
				if(m_pItems[i]->IsContainer())
				{
					bool result = TO< Container* >(m_pItems[i])->AddItemToFreeSlot(item, NULL);
					if(result)
						return ADD_ITEM_RESULT_OK;
				}
			}
			...
			else
			{
				for(uint32 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; i++)
				{
					if(m_pItems[i] != NULL && m_pItems[i]->IsContainer())
					{
						if(m_pItems[i]->GetProto()->BagFamily & proto->BagFamily)
						{
							int32 slot = TO< Container* >(m_pItems[i])->FindFreeSlot();
							if(slot != ITEM_NO_SLOT_AVAILABLE)
							{
								result.ContainerSlot = static_cast<int8>(i);
...
void ItemInterface::HandleItemDurations()
{
	...
	for(uint32 i = EQUIPMENT_SLOT_START; i <= CURRENCYTOKEN_SLOT_END; ++i)
	{
		...
		if(item1 != NULL && item1->IsContainer())
		{
			...
			{
				Item* item2 = TO< Container* >(item1)->GetItem(static_cast< int16 >(j));
...
bool ItemInterface::SwapItems(int8 DstInvSlot, int8 DstSlot, int8 SrcInvSlot, int8 SrcSlot)
{
	...
	if(SrcInvSlot == DstInvSlot)  //in 1 bag
	{
		...
		{
			TO< Container* >(GetInventoryItem(SrcInvSlot))->SwapItems(SrcSlot, DstSlot);
		}
	}
	else
	{
