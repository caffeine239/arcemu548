		...
		if(!corpseTarget)
		{
			...
			if(unitTarget)
			{
				if(unitTarget->IsCreature() && unitTarget->IsPet() && unitTarget->IsDead())
				{
					...
					unitTarget->SetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS, 0);
...
void Spell::SpellEffectCreateItem(uint32 i)
{
	...
	if(p_caster != NULL)
	{
		...
		if((skill != NULL) && (skill->skillId == SKILL_ALCHEMY))
		{
			//Potion Master
		...
		if(skill != 0)
		{
			DetermineSkillUp(skill->skillId);
			...
			for(std::set<ProfessionDiscovery*>::iterator itr = objmgr.ProfessionDiscoveryTable.begin(); itr != objmgr.ProfessionDiscoveryTable.end(); itr++)
			{
				...
				if(spellid == pf->SpellId && p_caster->_GetSkillLineCurrent(skill->skillId) >= pf->SkillValue && !p_caster->HasSpell(pf->SpellToDiscover) && Rand(pf->Chance))
				{
...
void Spell::SpellEffectUseGlyph(uint32 i)
{
	...
	GlyphPropertiesEntry* gp_new = dbcGlyphPropertiesStore.LookupEntryForced(g_new);
	if(!gp_new)
		return;
	...
	if(g_old)
	{
		...
		{
			GlyphPropertiesEntry* gp_old = dbcGlyphPropertiesStore.LookupEntryForced(g_old);
			if(gp_old)
				p_caster->RemoveAura(gp_old->SpellID);
		}
	}

	GlyphSlotEntry* gs = dbcGlyphSlotStore.LookupEntryForced(p_caster->GetUInt32Value(PLAYER_FIELD_GLYPH_SLOTS_1 + m_glyphslot));
	if(gs)
	{
...
void Spell::SpellEffectSummonDeadPet(uint32 i)
{
	...
	if(pPet)
	{
		...
		pPet->SetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS, 0);
...
void Spell::SpellEffectSkinPlayerCorpse(uint32 i)
{
	...
	if(playerTarget && !corpse)
	{
		...
		playerTarget->SetFlag(OBJECT_FIELD_DYNAMIC_FLAGS, U_DYN_FLAG_LOOTABLE);
