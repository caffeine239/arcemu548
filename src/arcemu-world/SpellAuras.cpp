...
void Aura::Remove()
{
	...
	*************************************************************/
	...
	if(flag != 0)
		m_target->RemoveFlag(UNIT_FIELD_AURA_STATE, flag);
	...
	for(uint32 x = 0; x < 7; x++)
	{
		if(Flag & (((uint32)1) << x))
		{
			if(m_target->IsPlayer())
			{
				if(mod->m_amount > 0)
				{
					TO< Player* >(m_target)->BaseResistanceModPctPos[x] += amt;
				}
				else
				{
					TO< Player* >(m_target)->BaseResistanceModPctNeg[x] -= amt;
				}
				TO< Player* >(m_target)->CalcResistance(x);
...
void Aura::SpellAuraPeriodicDamage(bool apply)
{
	if(apply)
	{
			...
			case 12654:
				{
					...
					{
						dmg = float2int32(TO< Player* >(c)->m_casted_amount[SCHOOL_FIRE] * parentsp->eff[0].EffectBasePoints / 100.0f);
					}
					else if(c != NULL)
					{
						if(!dmg)
		...
		{
			m_target->SetFlag(UNIT_FIELD_AURA_STATE, AURASTATE_FLAG_IMMOLATE);
		}
		...
		{
			m_target->SetFlag(UNIT_FIELD_AURA_STATE, AURASTATE_FLAG_POISON);
		}
	}
	else if((m_flags & (1 << mod->i)) == 0)   //add these checks to mods where immunity can cancel only 1 mod and not whole spell
	{
		if(m_spellProto->BGR_one_buff_on_target & SPELL_TYPE_WARLOCK_IMMOLATE)
			m_target->RemoveFlag(UNIT_FIELD_AURA_STATE, AURASTATE_FLAG_IMMOLATE);
		//maybe poison aurastate should get triggered on other spells too ?
		else if(m_spellProto->c_is_flags & SPELL_FLAG_IS_POISON)  //deadly poison
		{
			m_target->RemoveFlag(UNIT_FIELD_AURA_STATE, AURASTATE_FLAG_POISON);
		}
	}
}
...
void Aura::EventPeriodicDamage(uint32 amount)
{
	...
	if(m_target->GetGUID() != m_casterGuid)	//don't use resist when cast on self-- this is some internal stuff
	{
		if(c != NULL)
		{
			uint32 amp = m_spellProto->eff[mod->i].EffectAmplitude;
			if(!amp)
...
void Aura::SpellAuraPeriodicHeal(bool apply)
{
	if(apply)
	{
		...
		if(val > 0)
		{
			sEventMgr.AddEvent(this, &Aura::EventPeriodicHeal, (uint32)val, EVENT_AURA_PERIODIC_HEAL, GetSpellProto()->eff[mod->i].EffectAmplitude, 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);

			if(GetSpellProto()->NameHash == SPELL_HASH_REJUVENATION || GetSpellProto()->NameHash == SPELL_HASH_REGROWTH)
			{
				m_target->SetFlag(UNIT_FIELD_AURA_STATE, AURASTATE_FLAG_REJUVENATE);
...
void Aura::EventPeriodicHeal(uint32 amount)
{
	...
	int amp = m_spellProto->eff[mod->i].EffectAmplitude;
	if(!amp)
...
void Aura::SpellAuraModAttackSpeed(bool apply)
{
	...
	if(m_target->IsPlayer())
	{
		if(apply)
		{
			TO< Player* >(m_target)->ModAttackSpeed(mod->m_amount, MOD_MELEE);
		}
		else
		{
			TO< Player* >(m_target)->ModAttackSpeed(-mod->m_amount, MOD_MELEE);
		}
		TO< Player* >(m_target)->UpdateStats();
	}
	else
	{
		if(apply)
		{
			mod->fixed_amount[0] = m_target->GetModPUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME, mod->m_amount);
			mod->fixed_amount[1] = m_target->GetModPUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME + 1, mod->m_amount);
			mod->fixed_amount[2] = m_target->GetModPUInt32Value(UNIT_FIELD_RANGED_ATTACK_ROUND_BASE_TIME, mod->m_amount);
...
void Aura::SpellAuraModStealth(bool apply)
{
	if(apply)
	{
		...
		if(m_spellProto->NameHash == SPELL_HASH_STEALTH)
			m_target->SetFlag(UNIT_FIELD_SHAPESHIFT_FORM, 0x1E000000); //sneak anim

		m_target->SetFlag(UNIT_FIELD_ANIM_TIER, 0x020000);
		if(m_target->IsPlayer())
			...
			for(uint32 x = MAX_POSITIVE_AURAS_EXTEDED_START; x < MAX_POSITIVE_AURAS_EXTEDED_END; x++)
			{
				if(m_target->m_auras[x] != NULL)
				{
					...
					else // if got immunity for slow, remove some that are not in the mechanics
					{
						for(int i = 0; i < 3; i++)
						{
							uint32 AuraEntry = m_target->m_auras[x]->GetSpellProto()->eff[i].EffectApplyAuraName;
	...
	else
	{
		...
		if(m_spellProto->NameHash != SPELL_HASH_VANISH)
		{
			...
			m_target->RemoveFlag(UNIT_FIELD_SHAPESHIFT_FORM, 0x1E000000);

			m_target->RemoveFlag(UNIT_FIELD_ANIM_TIER, 0x020000);
		...
		if((m_target->HasAurasWithNameHash(SPELL_HASH_MASTER_OF_SUBTLETY) || m_target->HasAurasWithNameHash(SPELL_HASH_OVERKILL)) && m_spellProto->NameHash == SPELL_HASH_STEALTH)
		{
			for(uint32 x = MAX_POSITIVE_AURAS_EXTEDED_START ; x < MAX_POSITIVE_AURAS_EXTEDED_END; x++)
			{
				if(m_target->m_auras[x] &&
				        ...
				        m_target->m_auras[x]->GetSpellProto()->eff[0].EffectApplyAuraName != SPELL_AURA_DUMMY)
				{
...
{
	SetPositive();
	if(m_spellProto->eff[mod->i].Effect == SPELL_EFFECT_APPLY_FRIEND_AREA_AURA)  // WTF is this crap? TODO clean this
		return;
...
void Aura::SpellAuraModInvisibilityDetection(bool apply)
{
	...
	if(m_target->IsPlayer())
		TO< Player* >(m_target)->UpdateVisibility();
}
...
void Aura::SpellAuraModResistance(bool apply)
{
	...
	if(m_target->IsPlayer())
	{
		for(uint32 x = 0; x < 7; x++)
		{
			...
			{
				if(mod->m_amount > 0)
					TO< Player* >(m_target)->FlatResistanceModifierPos[x] += amt;
				else
					TO< Player* >(m_target)->FlatResistanceModifierNeg[x] -= amt;
				TO< Player* >(m_target)->CalcResistance(x);
			}
		}
	}
...
void Aura::SpellAuraPeriodicTriggerSpellWithValue(bool apply)
{
	if(apply)
	{
		...
		float amptitude = static_cast<float>(GetSpellProto()->eff[mod->i].EffectAmplitude);
		Unit* caster = GetUnitCaster();
		uint32 numticks = GetSpellDuration(m_spellProto, caster) / m_spellProto->eff[mod->i].EffectAmplitude;
		if(caster != NULL)
		{
...
void Aura::SpellAuraPeriodicEnergize(bool apply)
{
	if(apply)
	{
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicEnergize, (uint32)mod->m_amount, (uint32)mod->m_miscValue,
		                   EVENT_AURA_PERIODIC_ENERGIZE, GetSpellProto()->eff[mod->i].EffectAmplitude, 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
...
void Aura::EventPeriodicEnergize(uint32 amount, uint32 type)
{
	...
	ARCEMU_ASSERT(POWER_TYPE <= UNIT_FIELD_POWER + 4);
...
void Aura::SpellAuraModRoot(bool apply)
{
	if(apply)
	{
		...
		if(GetSpellProto()->School == SCHOOL_FROST && !m_target->asc_frozen++)
			m_target->SetFlag(UNIT_FIELD_AURA_STATE, AURASTATE_FLAG_FROZEN);
	...
	else if((m_flags & (1 << mod->i)) == 0)   //add these checks to mods where immunity can cancel only 1 mod and not whole spell
	{
		...
		if(GetSpellProto()->School == SCHOOL_FROST && !--m_target->asc_frozen)
			m_target->RemoveFlag(UNIT_FIELD_AURA_STATE, AURASTATE_FLAG_FROZEN);
	}
}
...
void Aura::SpellAuraModStat(bool apply)
{
	...
	if(stat == -1)   //all stats
	{
		if(m_target->IsPlayer())
		{
			...
			{
				if(mod->m_amount > 0)
					TO< Player* >(m_target)->FlatStatModPos[x] += val;
				else
					TO< Player* >(m_target)->FlatStatModNeg[x] -= val;

				TO< Player* >(m_target)->CalcStat(x);
			}

			TO< Player* >(m_target)->UpdateStats();
			TO< Player* >(m_target)->UpdateChances();
		}
		else if(m_target->IsCreature())
		{
			for(uint8 x = 0; x < 5; x++)
	...
	else if(stat >= 0)
	{
		...
		{
			if(mod->m_amount > 0)
				TO< Player* >(m_target)->FlatStatModPos[ mod->m_miscValue ] += val;
			else
				TO< Player* >(m_target)->FlatStatModNeg[ mod->m_miscValue ] -= val;

			TO< Player* >(m_target)->CalcStat(mod->m_miscValue);
...
void Aura::SpellAuraModSkill(bool apply)
{
	if(m_target->IsPlayer())
	{
		...
		{
			SetPositive();
			TO< Player* >(m_target)->_ModifySkillBonus(mod->m_miscValue, mod->m_amount);
		}
		else
			TO< Player* >(m_target)->_ModifySkillBonus(mod->m_miscValue, -mod->m_amount);
...
void Aura::SpellAuraModIncreaseHealth(bool apply)
{
	...
	if(m_target->IsPlayer())
	{
		...
		TO< Player* >(m_target)->SetHealthFromSpell(TO< Player* >(m_target)->GetHealthFromSpell() + amt);
		TO< Player* >(m_target)->UpdateStats();
		if(apply)
...
void Aura::SpellAuraModIncreaseEnergy(bool apply)
{
	...
	{
		TO< Player* >(m_target)->SetManaFromSpell(TO< Player* >(m_target)->GetManaFromSpell() + amount);
	}
}
...
void Aura::SpellAuraModShapeshift(bool apply)
{
	...
	SpellShapeshiftFormEntry* ssf = dbcSpellShapeshiftForm.LookupEntry(mod->m_miscValue);
	if(ssf == NULL)
		return;
			...
			for(uint32 x = MAX_POSITIVE_AURAS_EXTEDED_START; x < MAX_POSITIVE_AURAS_EXTEDED_END; x++)
			{
				if(m_target->m_auras[x] != NULL)
				{
					...
					else // if got immunity for slow, remove some that are not in the mechanics
					{
						for(int i = 0; i < 3; i++)
						{
							if(m_target->m_auras[x]->GetSpellProto()->eff[i].EffectApplyAuraName == SPELL_AURA_MOD_DECREASE_SPEED || m_target->m_auras[x]->GetSpellProto()->eff[i].EffectApplyAuraName == SPELL_AURA_MOD_ROOT)
							{
								m_target->m_auras[x]->Remove();
								break;
							}
	...
	if(apply)
	{
		for(uint32 x = MAX_POSITIVE_AURAS_EXTEDED_START; x < MAX_POSITIVE_AURAS_EXTEDED_END; x++)
		{
			...
			if(m_target->m_auras[x] && m_target->m_auras[x]->GetSpellId() != 41425 && m_target->m_auras[x]->GetSpellId() != 25771)
				if(m_target->m_auras[x]->GetSpellProto()->SpellCategories.DispelType == (uint32)mod->m_miscValue)
					m_target->m_auras[x]->Remove();
		}
	}
}
...
void Aura::SpellAuraProcTriggerSpell(bool apply)
{
	if(apply)
	{
		...
		spellId = GetSpellProto()->eff[mod->i].EffectTriggerSpell;
		if(spellId == 0)
		{
		...
		// Initialize mask
		groupRelation[0] = GetSpellProto()->eff[mod->i].EffectSpellGroupRelation[0];
		groupRelation[1] = GetSpellProto()->eff[mod->i].EffectSpellGroupRelation[1];
		groupRelation[2] = GetSpellProto()->eff[mod->i].EffectSpellGroupRelation[2];
	...
	{
		// Find spell of effect to be triggered
		uint32 spellId = GetSpellProto()->eff[mod->i].EffectTriggerSpell;
		if(spellId == 0)
		{
			LOG_DEBUG("Warning! trigger spell is null for spell %u", GetSpellProto()->Id);
			return;
...
void Aura::SpellAuraModCritPerc(bool apply)
{
	if(p_target != NULL)
	{
		if(apply)
		{
			...
			md.wclass = GetSpellProto()->SpellEquippedItems.EquippedItemClass;
			md.subclass = GetSpellProto()->SpellEquippedItems.EquippedItemSubClass;
...
void Aura::SpellAuraPeriodicLeech(bool apply)
{
	if(apply)
	{
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicLeech, amt,
		                   EVENT_AURA_PERIODIC_LEECH,	GetSpellProto()->eff[mod->i].EffectAmplitude, 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
...
void Aura::EventPeriodicLeech(uint32 amount)
{
	...
	int amp = sp->eff[mod->i].EffectAmplitude;
	if(!amp)
	...
	uint32 heal_amount = float2int32(amount * sp->eff[mod->i].Effect); // was eff[mod->i].EffectMultipleValue -- it's gone?
		...
		case 19937:
			{
				if(apply)
				{
					...
					m_target->SetUInt32Value(UNIT_FIELD_DISPLAY_ID, 7842);
				}
				else
				{
					m_target->SetUInt32Value(UNIT_FIELD_DISPLAY_ID, m_target->GetNativeDisplayId());
				}
			}
			break;
		...
		default:
			{
				...
				if(apply)
				{
					m_target->SetUInt32Value(UNIT_FIELD_DISPLAY_ID, displayId);
				}
				else
				{
					m_target->SetUInt32Value(UNIT_FIELD_DISPLAY_ID, m_target->GetNativeDisplayId());
...
void Aura::SpellAuraPeriodicHealthFunnel(bool apply)
{
	if(apply)
	{
		...
		sEventMgr.AddEvent(this, &Aura::EventPeriodicHealthFunnel, amt,
		                   EVENT_AURA_PERIODIC_HEALTH_FUNNEL, GetSpellProto()->eff[mod->i].EffectAmplitude, 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
}
...
void Aura::SpellAuraMechanicImmunity(bool apply)
{
	if(apply)
	{
		...
		if(m_spellProto->Id == 48020 && m_target->IsPlayer() && m_target->HasAura(62388))
		{
			...
			if(obj != NULL)
			{
				Player* ptarget = TO< Player* >(m_target);
...
void Aura::SpellAuraMounted(bool apply)
{
	...
	if(apply)
	{
		...
		if( p_target->mountvehicleid != 0 ){
			...
			p_target->SetFlag( UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_PLAYER_VEHICLE );
	...
	else
	{
		if( p_target->GetVehicleComponent() != NULL ){
			p_target->RemoveFlag( UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_PLAYER_VEHICLE );
...
void Aura::SpellAuraModDamagePercDone(bool apply)
{
	...
	if(p_target != NULL)
	{
		if(GetSpellProto()->SpellEquippedItems.EquippedItemClass == -1)  //does not depend on weapon
		{
			for(uint8 x = 0; x < SCHOOL_COUNT; x++)
			{
				if(mod->m_miscValue & ((uint32)1 << x))
...
void Aura::SpellAuraSplitDamage(bool apply)
{
	...
	if((m_spellProto->eff[mod->i].Effect == SPELL_EFFECT_APPLY_OWNER_AREA_AURA) &&
	        (caster != NULL) &&
	        (m_target != NULL) &&
	        caster->IsPet() &&
...
void Aura::SpellAuraModRegen(bool apply)
{
	...
	{
		SetPositive();
		sEventMgr.AddEvent(this, &Aura::EventPeriodicHeal1, (uint32)((this->GetSpellProto()->eff[mod->i].EffectBasePoints + 1) / 5) * 3,
...
void Aura::SpellAuraPeriodicTriggerDummy(bool apply)
{
	if(apply)
	{
		sEventMgr.AddEvent(this, &Aura::EventPeriodicTriggerDummy, EVENT_AURA_PERIODIC_DUMMY, m_spellProto->eff[mod->i].EffectAmplitude, 0, 0);
	}
	else
	{
...
void Aura::SpellAuraPeriodicDamagePercent(bool apply)
{
	if(apply)
	{
		...
		else*/
		{
			...
			sEventMgr.AddEvent(this, &Aura::EventPeriodicDamagePercent, dmg,
			                   EVENT_AURA_PERIODIC_DAMAGE_PERCENT, GetSpellProto()->eff[mod->i].EffectAmplitude, 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
		SetNegative();
	}
}
...
{
	int32 val = apply ? mod->m_amount : -mod->m_amount;
	uint32* AffectedGroups = GetSpellProto()->eff[mod->i].EffectSpellGroupRelation; // like that? 
...
{
	int32 v = spellInfo->eff[i].EffectBasePoints + 1;
	uint32* mask = spellInfo->eff[i].EffectSpellGroupRelation;
	uint8 type = static_cast<uint8>(spellInfo->eff[i].EffectMiscValue);
...
void Aura::SpellAuraAddClassTargetTrigger(bool apply)
{
	if(apply)
	{
		...
		procClassMask[0] = GetSpellProto()->eff[mod->i].EffectSpellGroupRelation[0];
		procClassMask[1] = GetSpellProto()->eff[mod->i].EffectSpellGroupRelation[1];
		procClassMask[2] = GetSpellProto()->eff[mod->i].EffectSpellGroupRelation[2];

		// Initialize mask
		groupRelation[0] = sp->eff[mod->i].EffectSpellGroupRelation[0];
		groupRelation[1] = sp->eff[mod->i].EffectSpellGroupRelation[1];
		groupRelation[2] = sp->eff[mod->i].EffectSpellGroupRelation[2];
		...
		m_target->AddProcTriggerSpell(sp->Id, GetSpellProto()->Id, m_casterGuid, GetSpellProto()->eff[mod->i].EffectBasePoints + 1, PROC_ON_CAST_SPELL, charges, groupRelation, procClassMask);
...
{
	if(apply)
		m_target->SetFlag(UNIT_FIELD_ANIM_TIER, STANDSTATE_FLAG_UNTRACKABLE);
	else
		m_target->RemoveFlag(UNIT_FIELD_ANIM_TIER, STANDSTATE_FLAG_UNTRACKABLE);
}
...
void Aura::SpellAuraModIncreaseHealthPerc(bool apply)
{
	...
	if(apply)
	{
		mod->fixed_amount[mod->i] = m_target->GetModPUInt32Value(UNIT_FIELD_MAX_HEALTH, mod->m_amount);
...
//			TO< Pet* >( m_target )->SetHealthFromSpell( ( ( Pet* )m_target )->GetHealthFromSpell() + mod->fixed_amount[mod->i] );
	...
	{
		m_target->ModMaxHealth(-mod->fixed_amount[mod->i]);
		if(m_target->GetUInt32Value(UNIT_FIELD_HEALTH) > m_target->GetUInt32Value(UNIT_FIELD_MAX_HEALTH))
			m_target->SetHealth(m_target->GetUInt32Value(UNIT_FIELD_MAX_HEALTH));
		if(p_target != NULL)
...
void Aura::SpellAuraModHaste(bool apply)
{
	...
	else
	{
		if(apply)
		{
			mod->fixed_amount[mod->i] = m_target->GetModPUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME, mod->m_amount);
			mod->fixed_amount[mod->i * 2] = m_target->GetModPUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME + 1, mod->m_amount);

			if((int32)m_target->GetUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME) <= mod->fixed_amount[mod->i])
				mod->fixed_amount[mod->i] = m_target->GetUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME);    //watch it, a negative timer might be bad ;)
			if((int32)m_target->GetUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME + 1) <= mod->fixed_amount[mod->i * 2])
				mod->fixed_amount[mod->i * 2] = m_target->GetUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME + 1); //watch it, a negative timer might be bad ;)
...
//		}
	...
	else
	{
		if(apply)
		{
			mod->fixed_amount[mod->i] = m_target->GetModPUInt32Value(UNIT_FIELD_RANGED_ATTACK_ROUND_BASE_TIME, mod->m_amount);
...
void Aura::SpellAuraPowerBurn(bool apply)
{
	...
	if(apply)
		sEventMgr.AddEvent(this, &Aura::EventPeriodicBurn, uint32(mod->m_amount), (uint32)mod->m_miscValue, EVENT_AURA_PERIODIC_BURN, GetSpellProto()->eff[mod->i].EffectAmplitude, 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
}
...
void Aura::SpellAuraIncreaseDamageTypePCT(bool apply)
{
	if(m_target->IsPlayer())
	{
		if(apply)
		{
			for(uint32 x = 0; x < 11; x++)
				if(mod->m_miscValue & (((uint32)1) << x))
					TO< Player* >(m_target)->IncreaseDamageByTypePCT[x + 1] += ((float)(mod->m_amount)) / 100;
			if(mod->m_amount < 0)
				SetNegative();
			else
				SetPositive();
...
void Aura::SpellAuraIncreaseCricticalTypePCT(bool apply)
{
	if(m_target->IsPlayer())
	{
		if(apply)
		{
			for(uint32 x = 0; x < 11; x++)
				if(mod->m_miscValue & (((uint32)1) << x))
					TO< Player* >(m_target)->IncreaseCricticalByTypePCT[x + 1] += ((float)(mod->m_amount)) / 100;
			if(mod->m_amount < 0)
				SetNegative();
			else
				SetPositive();
...
void Aura::SpellAuraIncreaseSpellDamageByAttribute(bool apply)
{
	...
	for(uint32 i = 0; i < 3; i++)
	{
		...
		if(m_spellProto->eff[i].EffectApplyAuraName == SPELL_AURA_INCREASE_SPELL_HEALING_PCT)
		{
			if(m_spellProto->eff[i].EffectMiscValue < 5)
				stat = m_spellProto->eff[i].EffectMiscValue;
			else
				return;
		}
	}
...
void Aura::SpellAuraAddFlatModifier(bool apply)
{
	...
	if(m_target->IsPlayer())
	{
		Pet* p = TO< Player* >(m_target)->GetSummon();
		if(p)
		{
...
void Aura::SpellAuraReduceEnemyMCritChance(bool apply)
{
	...
	if(apply)
	{
		...
		TO< Player* >(m_target)->res_M_crit_set(TO< Player* >(m_target)->res_M_crit_get() + mod->m_amount);
	}
	else
	{
		TO< Player* >(m_target)->res_M_crit_set(TO< Player* >(m_target)->res_M_crit_get() - mod->m_amount);
...
void Aura::SpellAuraReduceEnemyRCritChance(bool apply)
{
	...
	{
		//value is negative percent
		TO< Player* >(m_target)->res_R_crit_set(TO< Player* >(m_target)->res_R_crit_get() + mod->m_amount);
	}
	else
	{
		TO< Player* >(m_target)->res_R_crit_set(TO< Player* >(m_target)->res_R_crit_get() - mod->m_amount);
...
void Aura::SpellAuraMeleeHaste(bool apply)
{
	...
	else
	{
		if(apply)
		{
			...
			if((int32)m_target->GetUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME) <= mod->fixed_amount[0])
				mod->fixed_amount[0] = m_target->GetUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME);
			if((int32)m_target->GetUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME + 1) <= mod->fixed_amount[1])
				mod->fixed_amount[1] = m_target->GetUInt32Value(UNIT_FIELD_ATTACK_ROUND_BASE_TIME + 1);
...
//			TO< Player* >( m_target )->ModSkillBonusType(SKILL_TYPE_WEAPON, mod->m_amount);
			...
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_SWORDS, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_AXES, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_BOWS, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_GUNS, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_MACES, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_2H_SWORDS, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_STAVES, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_2H_MACES, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_2H_AXES, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_DAGGERS, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_CROSSBOWS, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_WANDS, mod->m_amount);
			TO< Player* >(m_target)->_ModifySkillBonus(SKILL_POLEARMS, mod->m_amount);
		}
		else
		{
//			TO< Player* >( m_target )->ModSkillBonusType(SKILL_TYPE_WEAPON, -mod->m_amount);
...
void Aura::SpellAuraIncreaseHitRate(bool apply)
{
	...
	TO< Player* >(m_target)->ModifyBonuses(SPELL_HIT_RATING, mod->m_amount, apply);
	TO< Player* >(m_target)->UpdateStats();
}
...
void Aura::SpellAuraIncreaseRageFromDamageDealtPCT(bool apply)
{
	...
	TO< Player* >(m_target)->rageFromDamageDealt += (apply) ? mod->m_amount : -mod->m_amount;
}
...
void Aura::SpellAuraIncreaseRating(bool apply)
{
	...
	if(mod->m_miscValue & 1)  //weapon skill
	{
		...
		for(uint32 y = 0; y < 20; y++)
			if(m_spellProto->SpellEquippedItems.EquippedItemSubClass & (((uint32)1) << y))
			{
				i = TO< Player* >(m_target)->m_wratings.find(y);
				if(i == TO< Player* >(m_target)->m_wratings.end())    //no prev
				{
					TO< Player* >(m_target)->m_wratings[y] = v;
				}
				else
				{
					i->second += v;
					if(i->second == 0)
						TO< Player* >(m_target)->m_wratings.erase(i);
				}
			}
	}
...
void Aura::SpellAuraFinishingMovesCannotBeDodged(bool apply)
{
	if(apply)
	{
		...
		TO< Player* >(m_target)->m_finishingmovesdodge = true;
	}
	else
	{
		if(!m_target->IsPlayer())
		...
		TO< Player* >(m_target)->m_finishingmovesdodge = false;
	}
}
...
void Aura::SpellAuraIncreaseMaxHealth(bool apply)
{
	...
	TO< Player* >(m_target)->SetHealthFromSpell(TO< Player* >(m_target)->GetHealthFromSpell() + amount);
	TO< Player* >(m_target)->UpdateStats();
}
...
void Aura::SendInterrupted(uint8 result, Object* m_caster)
{
	...
	if(m_caster->IsPlayer())
	{
		...
		data << uint8(result);
		TO< Player* >(m_caster)->GetSession()->SendPacket(&data);
	}
...
void Aura::SpellAuraReduceEffectDuration(bool apply)
{
	...
	{
		TO< Player* >(m_target)->MechanicDurationPctMod[mod->m_miscValue] += val;
	}
}
...
void Aura::SpellAuraAddHealth(bool apply)
{
	...
	{
		m_target->ModMaxHealth(-mod->m_amount);
		uint32 maxHealth = m_target->GetUInt32Value(UNIT_FIELD_MAX_HEALTH);
		if(m_target->GetUInt32Value(UNIT_FIELD_HEALTH) > maxHealth)
			m_target->SetUInt32Value(UNIT_FIELD_MAX_HEALTH, maxHealth);
	}
}
...
void Aura::SpellAuraIgnoreTargetAuraState(bool apply)
{
	...
	if(apply)
	{
		TO< Player* >(m_target)->ignoreAuraStateCheck = true;
	}
	else
	{
		TO< Player* >(m_target)->ignoreAuraStateCheck = false;
	}
}
...
void Aura::SpellAuraAllowOnlyAbility(bool apply)
{
	...
	if(apply)
	{
		...
		for(uint32 x = 0; x < 3; x++)
			p_target->m_castFilter[x] |= m_spellProto->eff[mod->i].EffectSpellGroupRelation[x];
	}
	else
	{
...
void Aura::SpellAuraIgnoreShapeshift(bool apply)
{
	...
	if(apply)
	{
		TO< Player* >(m_target)->ignoreShapeShiftChecks = true;
	}
	else
	{
		TO< Player* >(m_target)->ignoreShapeShiftChecks = false;
	}
}
...
void Aura::SpellAuraPhase(bool apply)
{
	...
	{
		if(m_target->IsPlayer())
			TO< Player* >(m_target)->GetSession()->SystemMessage("You can have only one phase aura!");
		Remove();
		return;
	}
...
bool Aura::DotCanCrit()
{
	...
	for(;;)
	{
		...
		uint32 i = 0;
		if(aura_sp->eff[1].EffectApplyAuraName == SPELL_AURA_ALLOW_DOT_TO_CRIT)
			i = 1;
		else if(aura_sp->eff[2].EffectApplyAuraName == SPELL_AURA_ALLOW_DOT_TO_CRIT)
			i = 2;

		if(aura_sp->SpellClassOptions.SpellFamilyName == sp->SpellClassOptions.SpellFamilyName &&
		        (aura_sp->eff[i].EffectSpellGroupRelation[0] & sp->SpellGroupType[0] ||
		         aura_sp->eff[i].EffectSpellGroupRelation[1] & sp->SpellGroupType[1] ||
		         aura_sp->eff[i].EffectSpellGroupRelation[2] & sp->SpellGroupType[2]))
		{
			found = true;
			break;
		}
...
void Aura::SpellAuraConvertRune(bool apply)
{
	...
	if(apply)
	{
		...
		for(uint8 i = 0; i < MAX_RUNES && count; ++i)
			...
			{
				dk->ConvertRune(i, GetSpellProto()->eff[mod->i].EffectMiscValueB);
				--count;
			}
	}
	else
	{
		uint8 count = mod->m_amount;
		for(uint8 i = 0; i < MAX_RUNES && count; ++i)
			if(dk->GetRuneType(i) == GetSpellProto()->eff[mod->i].EffectMiscValueB)
			{
