...
bool ChatHandler::HandleNpcInfoCommand(const char* args, WorldSession* m_session)
{
	...
	uint32 dynflags = crt->GetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS);
...
//	char* infos = new char[128];
	...
	if(sess->HasFlag(ACCOUNT_FLAG_XPACK_03) && sess->HasFlag(ACCOUNT_FLAG_XPACK_02) && sess->HasFlag(ACCOUNT_FLAG_XPACK_01))
		client = "TBC, WotLK and Cataclysm";
	else if(sess->HasFlag(ACCOUNT_FLAG_XPACK_02) && sess->HasFlag(ACCOUNT_FLAG_XPACK_01))
